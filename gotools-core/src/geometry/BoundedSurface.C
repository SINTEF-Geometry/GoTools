...
//===========================================================================
{
	...
	try {
	  ...
	  {
	    LOG_INFO("Failed deciding whether point was inside domain. "); // Replaced LOG_INFO streaming with LOG_()
...
//===========================================================================
{
  double ptol = 1.0e-8;
	...
	LOG_INFO("Input pts not member of the same loop!"); // Replaced LOG_INFO streaming with LOG_()
    }
	...
	if (bd_cv.get() == 0) {
	    LOG_INFO("Wasn't expecting this ..."); // Replaced LOG_INFO streaming with LOG_()
	}
...
//===========================================================================
{
	...
	for (int kj = 0; kj < boundary_loops_[ki]->size(); ++kj) {
		...
		if(!(scv->underlyingSurface() == surface_)) {
		    LOG_INFO("The boundary curves lie on the wrong surface!"); // Replaced LOG_INFO streaming with LOG_()
		}
		...
		if (pcv) {
			...
			LOG_INFO("The parameter curve is not a spline curve, so we can't switch " // Replaced LOG_INFO streaming with LOG_()
			      "U and V coordinates.");
		    }
		}
	    }
...
//===========================================================================
{
  ...
  for (size_t ki = 0; ki < boundary_loops_.size(); ++ki)
	...
	LOG_INFO("Expecting a CurveOnSurface."); // Replaced LOG_INFO streaming with LOG_()
...
// 	= dynamic_pointer_cast<SplineSurface, ParamSurface>(surface_);
//     LOG_INFO("Expecting underlying surface to be a spline surface."); // Replaced LOG_INFO streaming with LOG_()
...
//===========================================================================
{
    ...
    LOG_INFO("Expecting iso curve to be connected..."); // Replaced LOG_INFO streaming with LOG_()
...
//===========================================================================
{
    ...
    if (0)//valid_state_ != 1)
    {
	LOG_INFO("valid_state_: " + std::to_string(valid_state_) + ", par_cv_missing: " + std::to_string(par_cv_missing) + ", cv_match_ok: " + std::to_string(cv_match_ok) + ", loop_gaps_ok: " + std::to_string(loop_gaps_ok) + ", loop_order_ok: " + std::to_string(loop_order_ok)); // Replaced LOG_INFO streaming with LOG_()
    }
#endif
}
...
#ifdef SBR_DBG
    LOG_DEBUG("Must fix invalid surface! valid_state_ = " + std::to_string(valid_state_)); // Replaced LOG_DEBUG streaming with LOG_()
#endif
...
//===========================================================================
{
...
#ifdef SBR_DBG
    if (!analyze)
		LOG_DEBUG("Invalid loop!"); // Replaced LOG_DEBUG streaming with LOG_()
#endif
	...
	if (!valid_loop) {
		...
		LOG_INFO("Failed fixing invalid loop! max_gap = " + std::to_string(max_gap) + ", epsgeo = " + std::to_string(boundary_loops_[ki]->getSpaceEpsilon())); // Replaced LOG_INFO streaming with LOG_()
		break;
	    }
	}
    }
	...
	LOG_INFO("Failed fixing loop gap(s). BoundedSurface is still invalid."); // Replaced LOG_INFO streaming with LOG_()
	return false;
    }
}
...
//===========================================================================
{
	...
	if ((*loop)[ki]->instanceType() == Class_CurveOnSurface) {
		...
		{   // If verified_closed we should check if the seed should switch side.
                    ...
                    {
                        LOG_INFO("Seed is outside the domain!"); // Replaced LOG_INFO streaming with LOG_()
                    }
		    local_seed = seed;
		}
		// if (kj == 0)
...
//===========================================================================
{
	...
	LOG_INFO("Unexpected valid_state_ = " + std::to_string(valid_state_)); // Replaced LOG_INFO streaming with LOG_()
...
//    const double int_tol = GoTools::spaceEpsilon();
	...
	try {
	    ...
	    LOG_INFO("Failed analyzing loop."); // Replaced LOG_INFO streaming with LOG_()
	    return false;
	}
	...
	for (kj = 0; kj < boundary_loops_.size(); ++kj) {
	    ...
	    if (pcurve.get() == 0) {
		LOG_INFO("Missing parameter curve!"); // Replaced LOG_INFO streaming with LOG_()
		...
		LOG_INFO("Caught exception! Nothing more we can do ..."); // Replaced LOG_INFO streaming with LOG_()
		return false;
	    }
	}
...
// 		<< lies_inside_loop[ki].size());
	...
	{
// 	    valid_state_ += -2;
	    LOG_INFO(std::to_string(boundary_loops_.size()) + " loops in total. Found " + std::to_string(nmb_outer_loops) + " outer loops! BoundedSurface invalid. surface_->classtype(): " + std::to_string(surface_->instanceType())); // Replaced LOG_INFO streaming with LOG_()
	    return false;
	}
	else {
	    LOG_INFO(std::to_string(boundary_loops_.size()) + " loops in total. Failed finding one outer loop (" + std::to_string(nmb_outer_loops) + ")! BoundedSurface invalid."); // Replaced LOG_INFO streaming with LOG_()
// 	    MESSAGE("No success in finding a single outer loop!");
    ...
    if (outer_index != 0) {
	if (analyze) {
	    LOG_INFO("First loop is not the outer loop, BoundedSurface invalid."); // Replaced LOG_INFO streaming with LOG_()
	    return false;
	} else {
...
// 		cv_consistent =
		...
		if (max_trace_diff < max_tol_mult*space_eps) {
		    ...
		    same_trace = cv_on_sf->sameTrace(space_eps,
						     nmb_seg_samples);
		    if (!same_trace)
			LOG_INFO("Strange, this should not happen ..."); // Replaced LOG_INFO streaming with LOG_()
		} else {
		    if (max_trace_diff > 1.0) // Rather arbitrary. But we want to detect bugs, not bad input tolerance.
		    { 
			LOG_INFO("Deviation too large, epsgeo: " + std::to_string(space_eps) + ", max_trace_diff: " + std::to_string(max_trace_diff)); // Replaced LOG_INFO streaming with LOG_()
		    }
		}
	    }
    ...
    //===========================================================================
{
	...
	if (joints.size() == 2) {
		...
		LOG_INFO("Failed extracting subcurve on [" + std::to_string(joints[i]) + ", " + std::to_string(joints[i+1]) + "]."); // Replaced LOG_INFO streaming with LOG_()
