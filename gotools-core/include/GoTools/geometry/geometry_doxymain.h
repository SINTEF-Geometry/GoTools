/**
\page geometry_doc GoTools gotools-core Geometry

All GoTools modules depend on gotools-core. The module contains representation of 
...
\em Creators and \em tesselator contains
functionality for generating curves and surfaces by approximation,
blending, etc., and for making tesselations, respectively.
...
All geometric objects are of type \beinlink \link GeomObject.h GeomObject\endlink. 
...
\beginlink \link ParamCurve.h ParamCurve\endlink 
...
{\it control points} \f${\bf p}_i\f$. For example, if the dimension of the
control points
...
but both SISL and GoTools allow for higher dimensions.

A B-spline curve is a linear combination of a sequence of 
\beginlink \link BsplineBasis.h B-splines\endlink
...
\arg \c \f$dim\f$: The dimension of the underlying Euclidean space,
              \f$1,2,3,\ldots\f$.
\arg \c \f$n\f$: The number of vertices (also the number of B-splines)
\arg \c \f$k\f$: The order of the B-splines.
...
- The knot vector must be non-decreasing: \f$t_i \le t_{i+1}\f$.
      ...
      \f$t_i < t_{i+k}\f$.
- The number of vertices should be greater than or equal
...
vector, B-spline curves also have the {\it endpoint property}:
...
The control points \f${\bf p}_i\f$ define the vertices
...
and thereby increasing the number of vertices. This can be seen in the next
figure.
...
In GoTools, SplineCurve inherites ParamCurve and has thereby the functionality
...
\beginlink \link SplineSurface.h SplineSurface\endlink.
...
The control points \f${\bf p}_{i,j}\f$ form a {\it control net} as shown in
figure~\ref{surf1}.
...
SplineSurface inherites ParamSurface in the GoTools data structure and 
...
As shown in figure~\ref{fig::geom_classes}, a spline curve is not the only
...
\arg \c \beginlink \link BoundedCurve.h BoundedCurve\endlink 
...
required the existence of one such curve, in particular the parameter curve
is requested. It exists functions that compute the missing curve
from the existing one.

\subsection geom_sec4_3 Elementary Curve
An \beginlink \link ElementaryCurve.h elementary curve\endlink 
...
finite extension. However, the knowledge about a curve being elementary
implies that some properties of the curve already is known. Operations 
involving this curve may
be made more efficiently. The elementary curves is of the types:
- \beginlink \link Circle.h Circle\endlink 
...
parameterization.
- \beginlink \link Ellipse.h Ellipse\endlink 
...
surfaces than spline surfaces, see figure~\ref{fig::geom_classes}.
...
\beginlink \link BoundedSurface.h A bounded surface\endlink 
...
\subsection geom_sec5_2 Elementary Surface
Also \beginlink \link ElementarySurface.h elementary surfaces\endlink 
...
- \beginlink \link Cone.h Cone\endlink 
...
- \beginlink \link Cylinder.h Cylinder\endlink 
...
- \begnlink \link Torus.h Torus\endlink 
...
construction. They partly overlap. The SISL geometry construction methods are
described in the SISL manual. 
...
<ul>
<li> \beginlink \link HermiteInterpolator.h HermiteInterpolator\endlink 
...
<li> \beginlink \link SplineInterpolator.h SplineInterpolator\endlink 
...
<li> \beginlink \link SplineApproximator.h SplineApproximator\endlink 
Approximation in the least squares sense. 
Parameterized points and a spline space must be given. This class and the
two proceeding ones inherite the same class, Interpolator, and share parts
of the interface.
<li> \beginlink \link ApproxCurve.h ApproxCurve\endlink 
...
creators.
<li> \beginlink \link SmoothCurve.h SmoothCurve\endlink 
...
on a given curve.
<li> \beginlink \link AdaptCurve.h AdaptCurve\endlink 
...
space refinement.
<li> \beginlink \link ApproxCrvToSeqs.h ApproxCrvToSeqs\endlink 
...
<li> \beginlink \link HermiteAppC.h HermiteAppC\endlink 
Approximate an evaluator based curve represented by a
sub class of the class \beginlink \link EvalCurve.h EvalCurve\endlink, 
by a spline curve. An hermite method is 
used, thus a \f$C^1\f$ continuous curve will be generated. The existing evaluator
based curves are
<ol>
      <li> \beginlink \link ProjectCurve.h A curve in the parameter domain \endlink
...
\beginlink \link ProjectIntersectionCurve.h surface-surface intersection curve\endlink, 
the curve
...
\beginlink \link SpaceIntCrv.h intersection curve\endlink.
<li> Given a \beginlink \link LiftCurve.h curve in the parameter domain \endlink
of a surface, the geometry space curve is evaluated.
<li> Improve the accuracy of an existing \beginlink \link TrimCurve.h trimming
curve\endlink.
<li> Represent \beginlink \link CrossTangentOffset.h an offset curve\endlink 
from a given space curve. The direction of
the offset is given as an expression of corresponding curves. 
</ol>
<li> \beginlink \link HermiteAppS.h HermiteAppS\endlink 
Approximate an evaluator based curve set represented by a
sub class of the class \beginlink \link EvalCurveSet.h EvalCurveSet\endlink. 
...
tangent curve, \beginlink \link CrossTanOffDist.h the parameter curve \endlink
...
<li> Given an \beginlink \link IntCrvEvaluator.h intersection curve\endlink
...
<ul>
<li> \beginlink \link SweepSurfaceCreator.h SweepSurfaceCreator\endlink 
The class lies
in geometry. The class contains two methods; sweep a curve along another
curve to create a surface and rotational sweep.
<li> \beginlink \link ApproxSurf.h ApproxSurf\endlink 
...
parameter iteration.
<li> \beginlink \link SmoothSurf.h SmoothSurf\endlink 
...
<li> \beginlink \link CoonsPatchGen.h CoonsPatchGen\endlink 
...
<li> \beginlink \link HahnsSurfaceGen.h HahnsSurfaceGen\endlink 
...
tesselation is stored in \beginlink \link GenericTriMesh.h GenericTriMesh\endlink, 
\beginlink \link LineStrip.h LineStrip\endlink, 
\beginlink \link QuadMesh.h QuadMesh\endlink or
\beginlink \link RegularMesh.h RegularMesh\endlink 
that all inherite the abstract class GeneralMesh. The tesselators
in this module relate to a given resolution in each parameter direction of
the geometry object.

A curve is tesselated by the class 
\beginlink \link CurveTesselator.h CurveTesselator\endlink 
...
\beginlink \link RectGridTesselator.h RectGridTesselator\endlink and the tesselator
...
\beginlink \link ParametricSurfaceTesselator.h ParametricSurfaceTesselator\endlink 
