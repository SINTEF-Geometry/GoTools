...
{
  if (argc != 6) {
    std::cout << "Usage: input rainfall data (.mat/.csv),  output info (.txt), output surface(.g2), tolerance, maximum number iterations " << std::endl;
    return -1;
  }

 // Read input arguments
  std::ifstream input_rain(argv[1]);
  std::ofstream out_info(argv[2]);
  std::ofstream out_sf(argv[3]);
  ...
  for (kj=0; kj<nmb_rain; ++kj)
      ...
      for (ki=0; ki<nmb_loc; ++ki)
	{
      ...
      // double fac = 1.0/reduce_fac;
      // surf->setParameterDomain(fac*domain[0] + mid[0], fac*domain[1] + mid[0],
      // 			       fac*domain[2] + mid[1], fac*domain[3] + mid[1]);
