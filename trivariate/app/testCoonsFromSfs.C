...
int main(int argc, char* argv[] )
{
  if (argc != 4)
    {
      cout << "Usage: " << argv[0] << " infile outfile tolerance" << endl;
      exit(-1);
    }
  ...
  vector<shared_ptr<SplineSurface> > surfs(6);
  for (int ki=0; ki<6; ++ki)
    {
      ObjectHeader head;
      is >> head;
      surfs[ki] = shared_ptr<SplineSurface>(new SplineSurface());
      surfs[ki]->read(is);
    }

  SplineVolume* volnew = 
    Go::CoonsPatchVolumeGen::createCoonsPatch(surfs[0].get(), surfs[1].get(), 
					      surfs[2].get(), surfs[3].get(), 
					      surfs[4].get(), surfs[5].get(),
					      tol);
